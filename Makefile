...
jolt-regression-matrix-self-test:
	...
	@report=$$(mktemp); \
	  ...
	  bb -e '(let [r (read-string (slurp (first *command-line-args*)))] (assert (:ok? r)) (assert (= {:pass 14 :fail 14 :xpass 0 :error 0} (:summary r))))' "$$report"
	@report=$$(mktemp); \
	  trap 'rm -f "$$report"' EXIT; \
	  JOLT_UNFIXED="$$PWD/test/fixtures/regression-matrix/upstream-fixed-jolt" \
	  JOLT_FIXED="$$PWD/test/fixtures/regression-matrix/fixed-jolt" \
	    bb regressions/jolt/run.bb >"$$report"; \
	  bb -e '(let [r (read-string (slurp (first *command-line-args*)))] (assert (:ok? r)) (assert (= {:pass 14 :fail 0 :xpass 14 :error 0} (:summary r))))' "$$report"
