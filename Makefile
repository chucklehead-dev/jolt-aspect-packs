...
jolt-regression-matrix-self-test:
	...
	@report=$$(mktemp); \
	  ...
	  bb -e '(let [r (read-string (slurp (first *command-line-args*)))] (assert (:ok? r)) (assert (= {:pass 10 :fail 10 :xpass 0 :error 0} (:summary r))))' "$$report"
	@report=$$(mktemp); \
	  trap 'rm -f "$$report"' EXIT; \
	  JOLT_UNFIXED="$$PWD/test/fixtures/regression-matrix/upstream-fixed-jolt" \
	  JOLT_FIXED="$$PWD/test/fixtures/regression-matrix/fixed-jolt" \
	    bb regressions/jolt/run.bb >"$$report"; \
	  bb -e '(let [r (read-string (slurp (first *command-line-args*)))] (assert (:ok? r)) (assert (= {:pass 10 :fail 0 :xpass 10 :error 0} (:summary r))))' "$$report"
