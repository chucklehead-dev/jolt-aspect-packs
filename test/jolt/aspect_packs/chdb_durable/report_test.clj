(ns jolt.aspect-packs.chdb-durable.report-test
...
(defn -main [report-path]
  (let [report (edn/read-string (slurp report-path))
    ...
    (when-not (false? (:control-enabled? report))
      (throw (ex-info "Durable history enabled control advice" {})))
    ...
    (doseq [[id entry arity] expected
      ...
      (when-not (= ['jolt.aspect-packs.chdb-durable.provider/aspect-provider]
                   (mapv :provider (:consumers aspect)))
        (throw (ex-info "wrong Durable history consumer" {:aspect id})))
      (when-not (every? #(= :args-v1 (:contract %)) (:consumers aspect))
        (throw (ex-info "wrong Durable advice contract" {:aspect id}))))
    (println "ASPECT-PACK-CHDB-DURABLE-REPORT OK")))
