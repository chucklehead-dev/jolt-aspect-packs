...
alts behavior from issue #4's constructor defect. Issues #12 through #14 use
the exposed `jolt.scheme` seam to observe native notification, pair ownership,
and controlled callback scheduling without the original native test harness.
Their catalog provenance labels each internal boundary explicitly.
