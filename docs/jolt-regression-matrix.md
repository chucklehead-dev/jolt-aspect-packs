...
alts behavior from issue #4's constructor defect. Issues #12 and #13 use the
exposed `jolt.scheme` seam to observe native notification and pair-ownership
boundaries without the original native test harness. Their catalog provenance
labels each internal boundary explicitly.
...
snapshot has 40 open fork-fixed issues, 16 with portable cases and 24 with named
